...
output_path = "/shared/davidsalvador/data/morphocut-test"

# name of target image
image_name = "Pia1.2016-08-02.1631+N38_hc._fx.tif"
...
with Pipeline() as p:
    ...
    pattern = Call( os.path.join, base_path, os.path.join("images",image_name) )
    ...
    with Pipeline():
        ...
        roi_name = Format(
            "{:d}-{}-{:d}.tif", running_number, source_basename, roi_label
        )
    ...
    # Store results
    EcotaxaWriter(os.path.join(output_path, "archive.zip"), (roi_name, roi_image), meta)
...
# unzip output - search for output in output_path
with zipfile.ZipFile(os.path.join(output_path, "archive.zip"),"r") as zip_ref:
