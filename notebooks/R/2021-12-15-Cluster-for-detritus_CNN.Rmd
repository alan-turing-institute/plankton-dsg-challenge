---
title: "Cluster for detritus (CNN)"
author: "Sari Giering (15 Dec 2021)"
output:
  html_document: 
    keep_md: no
...
2021-12-15 Renamed from '2021-12-09 NMDS'. Now mentions clusters.  
...
Unfortunately, we did not have time to extract the CNN features in a format that can be used to extract the corresponding images.
...
```{r load_data}
  # define project root
  library(here)
  i_am("notebooks/R/2021-12-15-Cluster-for-detritus_CNN.Rmd")

  # load low-level features
  dat <- read.table(here("data", "processed", "morphocut", "ecotaxa_export.tsv"), comment.char = "[", header = T)

  # ---- add label1 ----
  # read index file
  index_all <- read.csv(here("data", "processed", "clean-index", "index-clean.csv"), header = TRUE)
...
```{r pca}
  ...
  # calculate PCA
  pca <- PCA(mat_scaled, graph = FALSE)
```
...
```{r elbow}
  ...
  fviz_nbclust(mat_scaled, kmeans, method = "wss", k.max = 15) + theme_minimal() + ggtitle("kmeans CORE: the Elbow Method")
```

7 clusters look good to me.

The Elbow Curve method is (sometimes) helpful because it shows how increasing the number of the clusters contribute separating the clusters in a meaningful way, not in a marginal way. The first 3 clusters are clearly important; beyond cluster 7 a clear cut-off is not obvious.
...
```{r make_collage}
  ...
  for(TYPE in unique(detr_sub$cluster)) {
    ...
    if(n <= 25) INDEX <- 1:n else INDEX <- sample(1:n, 25)
